...
function AnnotationRuntimeCard() {
  ...
  return <ProCard title="标注模型与 GPU 协调" extra={<Space>{settings ? (settings.provider === 'local' ? <Tag color="green">本地 Qwen2.5-VL</Tag> : <Tag color="blue">Cloud</Tag>) : null}<Button loading={loading} onClick={loadRuntime}>刷新</Button></Space>}><Form form={form} layout="vertical" onFinish={saveSettings} initialValues={{ provider: 'cloud' }}><Row gutter={16}><Col xs={24} md={8}><Form.Item name="provider" label="标注供应商" rules={[{ required: true }]}><Select options={[{ value: 'local', label: '本地 Qwen2.5-VL-7B / vLLM' }, { value: 'cloud', label: '云端 / Azure Foundry' }]} /></Form.Item></Col><Col xs={24} md={8}><Descriptions column={1} bordered size="small"><Descriptions.Item label="vLLM 服务">{statusTag(runtime?.status || '未知')}</Descriptions.Item><Descriptions.Item label="Endpoint">{runtime?.endpoint || '-'}</Descriptions.Item></Descriptions></Col><Col xs={24} md={8}><Space wrap><Button loading={vllmAction === 'start'} onClick={() => controlVllm('start')}>启动 vLLM</Button><Button loading={vllmAction === 'stop'} onClick={() => controlVllm('stop')}>停止 vLLM</Button><Button loading={vllmAction === 'restart'} onClick={() => controlVllm('restart')}>重启 vLLM</Button></Space></Col></Row><Divider orientation="left">本地 Qwen2.5-VL</Divider><Row gutter={16}><Col xs={24} md={12}><Form.Item name={['local', 'endpoint']} label="Endpoint"><Input placeholder="http://127.0.0.1:8000/v1" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['local', 'model']} label="模型"><Input placeholder="/data/models/qwen2.5-vl-7b-instruct" /></Form.Item></Col></Row><Divider orientation="left">云端 / Azure Foundry</Divider><Row gutter={16}><Col xs={24} md={12}><Form.Item name={['cloud', 'endpoint']} label="Endpoint"><Input placeholder="https://...cognitiveservices.azure.com/" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'deployment']} label="Deployment"><Input placeholder="gpt-4o" /></Form.Item></Col><Col xs={24} md={8}><Form.Item name={['cloud', 'resourceGroup']} label="Resource Group"><Input /></Form.Item></Col><Col xs={24} md={8}><Form.Item name={['cloud', 'accountName']} label="Account Name"><Input /></Form.Item></Col><Col xs={24} md={8}><Form.Item name={['cloud', 'apiVersion']} label="API Version"><Input /></Form.Item></Col></Row><Button type="primary" htmlType="submit" loading={saving}>保存标注配置</Button></Form></ProCard>;
}
...
function AnnotationPage() {
  ...
  return <PageContainer title="标注" subTitle="标注提示词和供应商配置；智能体标注会使用这里保存的设置。"><div className="page-stack"><ProCard title="标注供应商" extra={settings ? <Space>{settings.provider === 'local' ? <Tag color="green">本地 vLLM</Tag> : <Tag color="blue">Cloud</Tag>}<Text type="secondary">{settings.provider === 'local' ? settings.local?.model : settings.cloud?.deployment}</Text></Space> : null}><Form form={settingsForm} layout="vertical" onFinish={saveSettings} initialValues={{ provider: 'cloud' }}><Row gutter={16}><Col xs={24} md={8}><Form.Item name="provider" label="供应商" rules={[{ required: true }]}><Select options={[{ value: 'cloud', label: 'Cloud / Azure OpenAI' }, { value: 'local', label: '本地 vLLM' }]} /></Form.Item></Col><Col xs={24} md={16}><Alert type="info" showIcon message="数据集页的“智能体标注”会读取此配置" description="部署到 GPU VM 后请选择本地 vLLM，默认调用 VM 本机的 OpenAI-compatible vLLM 服务。" /></Col></Row><Divider orientation="left">本地 vLLM</Divider><Row gutter={16}><Col xs={24} md={12}><Form.Item name={['local', 'endpoint']} label="Endpoint"><Input placeholder="http://127.0.0.1:8000/v1" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['local', 'model']} label="模型"><Input placeholder="/data/models/qwen2.5-vl-7b-instruct" /></Form.Item></Col></Row><Divider orientation="left">Cloud</Divider><Row gutter={16}><Col xs={24} md={12}><Form.Item name={['cloud', 'endpoint']} label="Azure Endpoint"><Input placeholder="https://...cognitiveservices.azure.com/" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'deployment']} label="Deployment"><Input placeholder="gpt-4o" /></Form.Item></Col><Col xs={24} md={8}><Form.Item name={['cloud', 'resourceGroup']} label="Resource Group"><Input /></Form.Item></Col><Col xs={24} md={8}><Form.Item name={['cloud', 'accountName']} label="Account Name"><Input /></Form.Item></Col><Col xs={24} md={8}><Form.Item name={['cloud', 'apiVersion']} label="API Version"><Input /></Form.Item></Col></Row><Button type="primary" htmlType="submit">保存供应商配置</Button></Form></ProCard><ProCard title="中文 VLM 标注提示词"><Form layout="vertical"><Form.Item label="标注提示词"><TextArea rows={20} value={prompt} onChange={(event) => setPrompt(event.target.value)} /></Form.Item><Alert type="info" showIcon message="此提示词会作为智能体标注的基础提示词" description="运行标注时，系统会在这里保存的内容后追加当前数据集 trigger、分类枚举和结构化 JSON 输出约束。质量字段使用 0-100 数字评分，由智能体根据图片训练价值判断。" /><Divider /><Button type="primary" onClick={savePrompt}>保存提示词</Button></Form></ProCard></div></PageContainer>;
}
