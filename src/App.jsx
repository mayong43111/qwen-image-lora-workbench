...
function AnnotationRuntimeCard() {
  ...
  const [vllmAction, setVllmAction] = useState('');
  async function loadRuntime() {
    setLoading(true);
    try {
      const [settingsPayload, statusPayload] = await Promise.all([api('/api/annotation-settings'), api('/api/models/status')]);
      const nextSettings = settingsPayload.settings || {};
      setSettings(nextSettings);
      form.setFieldsValue(nextSettings);
      setRuntime(statusPayload.vllm?.runtime || {});
    } catch (error) {
      message.error(`读取标注运行时失败：${error.message}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function saveSettings(values) {
    setSaving(true);
    try {
      const payload = await api('/api/annotation-settings', { method: 'PUT', body: JSON.stringify(values) });
      setSettings(payload.settings || values);
      form.setFieldsValue(payload.settings || values);
      message.success('标注供应商配置已保存');
      loadRuntime();
    } catch (error) {
  ...
  async function controlVllm(action) {
    setVllmAction(action);
    try {
      const payload = await api(`/api/models/vllm/${action}`, { method: 'POST' });
      setRuntime(payload.vllm || {});
      message.success(`vLLM ${action === 'start' ? '启动' : action === 'stop' ? '停止' : '重启'}命令已执行`);
      loadRuntime();
    } catch (error) {
      message.error(`vLLM 控制失败：${error.message}`);
    } finally {
      setVllmAction('');
    }
  }
  async function activateProvider(providerValue) {
    const values = form.getFieldsValue(true);
    await saveSettings({ ...values, provider: providerValue });
  }
  const provider = settings?.provider || 'cloud';
  const cloud = settings?.cloud || {};
  const local = settings?.local || {};
  return <Form form={form} layout="vertical" onFinish={saveSettings} initialValues={{ provider: 'cloud' }}><div className="page-stack"><ProCard title="标注模型" extra={<Space>{provider === 'local' ? <Tag color="green">当前启用：本地 Qwen2.5-VL</Tag> : <Tag color="blue">当前启用：云端 Azure Foundry</Tag>}<Button loading={loading} onClick={loadRuntime}>刷新</Button></Space>}><Descriptions column={{ xs: 1, md: 3 }} bordered size="small"><Descriptions.Item label="当前供应商">{provider === 'local' ? '本地 Qwen2.5-VL' : '云端 Azure Foundry'}</Descriptions.Item><Descriptions.Item label="本地 vLLM">{statusTag(runtime?.status || '未知')}</Descriptions.Item><Descriptions.Item label="云端 API Key">{cloud.apiKeyConfigured ? <Tag color="green">已配置</Tag> : <Tag color="orange">未配置</Tag>}</Descriptions.Item></Descriptions></ProCard><Row gutter={[16, 16]}><Col xs={24} lg={12}><ProCard title="本地 Qwen2.5-VL" extra={provider === 'local' ? <Tag color="green">已启用</Tag> : null}><Descriptions column={1} bordered size="small"><Descriptions.Item label="Endpoint">{local.endpoint || '-'}</Descriptions.Item><Descriptions.Item label="模型">{local.model || '-'}</Descriptions.Item></Descriptions><Divider /><Row gutter={12}><Col xs={24} md={12}><Form.Item name={['local', 'endpoint']} label="Endpoint"><Input placeholder="http://127.0.0.1:8000/v1" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['local', 'model']} label="模型"><Input placeholder="/data/models/qwen2.5-vl-7b-instruct" /></Form.Item></Col></Row><Space wrap><Button type={provider === 'local' ? 'primary' : 'default'} loading={saving} onClick={() => activateProvider('local')}>启用本地 Qwen2.5-VL</Button><Button loading={vllmAction === 'start'} onClick={() => controlVllm('start')}>启动 vLLM 服务</Button><Button loading={vllmAction === 'stop'} onClick={() => controlVllm('stop')}>停止 vLLM</Button><Button loading={vllmAction === 'restart'} onClick={() => controlVllm('restart')}>重启 vLLM</Button></Space></ProCard></Col><Col xs={24} lg={12}><ProCard title="云端 / Azure Foundry" extra={provider === 'cloud' ? <Tag color="blue">已启用</Tag> : null}><Descriptions column={1} bordered size="small"><Descriptions.Item label="Endpoint">{cloud.endpoint || '-'}</Descriptions.Item><Descriptions.Item label="Deployment / Model">{cloud.deployment || '-'}</Descriptions.Item></Descriptions><Divider /><Row gutter={12}><Col xs={24} md={12}><Form.Item name={['cloud', 'endpoint']} label="Endpoint"><Input placeholder="https://...cognitiveservices.azure.com/" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'apiKey']} label="API Key"><Input.Password placeholder={cloud.apiKeyConfigured ? '已配置，留空则不修改' : '请输入 API key'} autoComplete="new-password" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'deployment']} label="Deployment / Model"><Input placeholder="gpt-4o" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'apiVersion']} label="API Version"><Input placeholder="2024-10-21" /></Form.Item></Col></Row><Space wrap><Button type={provider === 'cloud' ? 'primary' : 'default'} loading={saving} onClick={() => activateProvider('cloud')}>启用云端 Azure Foundry</Button><Button htmlType="submit" loading={saving}>保存云端配置</Button></Space></ProCard></Col></Row><Form.Item name="provider" hidden><Input /></Form.Item></div></Form>;
}
...
function AnnotationPage() {
  ...
  const provider = settings?.provider || 'cloud';
  const cloud = settings?.cloud || {};
  return <PageContainer title="标注" subTitle="标注提示词和供应商配置；智能体标注会使用这里保存的设置。"><div className="page-stack"><ProCard title="标注供应商" extra={settings ? <Space>{provider === 'local' ? <Tag color="green">当前启用：本地 vLLM</Tag> : <Tag color="blue">当前启用：云端</Tag>}<Text type="secondary">{provider === 'local' ? settings.local?.model : settings.cloud?.deployment}</Text></Space> : null}><Form form={settingsForm} layout="vertical" onFinish={saveSettings} initialValues={{ provider: 'cloud' }}><Row gutter={16}><Col xs={24} md={8}><Form.Item name="provider" label="当前启用" rules={[{ required: true }]}><Select options={[{ value: 'cloud', label: '云端 / Azure Foundry' }, { value: 'local', label: '本地 Qwen2.5-VL-7B / vLLM' }]} /></Form.Item></Col><Col xs={24} md={16}><Alert type="info" showIcon message="数据集页的“智能体标注”会读取此配置" description={provider === 'local' ? '当前使用本地 vLLM；标注前会自动启动本地 Qwen2.5-VL。' : '当前使用云端 endpoint + API key；不会读取 Azure resource group 或 account name。'} /></Col></Row><Divider orientation="left">本地 Qwen2.5-VL</Divider><Row gutter={16}><Col xs={24} md={12}><Form.Item name={['local', 'endpoint']} label="Endpoint"><Input placeholder="http://127.0.0.1:8000/v1" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['local', 'model']} label="模型"><Input placeholder="/data/models/qwen2.5-vl-7b-instruct" /></Form.Item></Col></Row><Divider orientation="left">云端 / Azure Foundry</Divider><Row gutter={16}><Col xs={24} md={12}><Form.Item name={['cloud', 'endpoint']} label="Endpoint"><Input placeholder="https://...cognitiveservices.azure.com/" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'apiKey']} label="API Key"><Input.Password placeholder={cloud.apiKeyConfigured ? '已配置，留空则不修改' : '请输入 API key'} autoComplete="new-password" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'deployment']} label="Deployment / Model"><Input placeholder="gpt-4o" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'apiVersion']} label="API Version"><Input placeholder="2024-10-21" /></Form.Item></Col></Row><Button type="primary" htmlType="submit">保存供应商配置</Button></Form></ProCard><ProCard title="中文 VLM 标注提示词"><Form layout="vertical"><Form.Item label="标注提示词"><TextArea rows={20} value={prompt} onChange={(event) => setPrompt(event.target.value)} /></Form.Item><Alert type="info" showIcon message="此提示词会作为智能体标注的基础提示词" description="运行标注时，系统会在这里保存的内容后追加当前数据集 trigger、分类枚举和结构化 JSON 输出约束。质量字段使用 0-100 数字评分，由智能体根据图片训练价值判断。" /><Divider /><Button type="primary" onClick={savePrompt}>保存提示词</Button></Form></ProCard></div></PageContainer>;
}
...
function TrainingPage({ datasets, refresh }) {
  ...
  async function submitTraining(values) {
    ...
    try {
      ...
      message.success(`训练任务已启动：${payload.lora?.name || 'LoRA'}`);
      refresh?.();
    } catch (error) {
  ...
  return <PageContainer title="训练" subTitle="选择数据集后直接准备训练数据，并启动 musubi-tuner LoRA 训练。"><div className="page-stack"><Steps current={dataset ? 2 : 0} items={[{ title: '选择数据集' }, { title: '检查图片和 caption' }, { title: '准备训练数据' }, { title: '开始训练' }]} /><Row gutter={[16, 16]}><Col xs={24} lg={10}><ProCard title="训练输入"><Form form={form} layout="vertical" initialValues={{ baseModel: 'Qwen Image', resolution: 1024, rank: 16, learningRate: '1e-4', batchSize: 1, seed: 42, strength: 0.8 }} onFinish={submitTraining}><Form.Item label="数据集" required><Select value={selectedDatasetId} onChange={setSelectedDatasetId} placeholder="选择数据集" options={datasets.map((item) => ({ value: item.id, label: `${item.name} / ${item.trigger}` }))} /></Form.Item><Form.Item name="name" label="LoRA 名称"><Input placeholder={dataset ? `${dataset.name} LoRA` : 'LoRA 名称'} /></Form.Item><Form.Item name="baseModel" label="基础模型"><Input /></Form.Item><Row gutter={12}><Col span={12}><Form.Item name="resolution" label="分辨率"><InputNumber min={512} max={2048} step={64} style={{ width: '100%' }} /></Form.Item></Col><Col span={12}><Form.Item name="steps" label="训练步数"><InputNumber min={1} max={20000} placeholder={estimate ? String(estimate.steps) : '自动'} style={{ width: '100%' }} /></Form.Item></Col></Row><Row gutter={12}><Col span={12}><Form.Item name="rank" label="LoRA Rank"><InputNumber min={4} max={128} style={{ width: '100%' }} /></Form.Item></Col><Col span={12}><Form.Item name="batchSize" label="Batch"><InputNumber min={1} max={16} style={{ width: '100%' }} /></Form.Item></Col></Row><Row gutter={12}><Col span={12}><Form.Item name="learningRate" label="学习率"><Input /></Form.Item></Col><Col span={12}><Form.Item name="seed" label="Seed"><InputNumber min={0} style={{ width: '100%' }} /></Form.Item></Col></Row><Form.Item name="gpuCommand" label="GPU VM 执行命令"><TextArea rows={3} placeholder="留空时由系统生成 musubi-tuner Qwen Image LoRA 命令" /></Form.Item><Button type="primary" block htmlType="submit" loading={submitting} disabled={!dataset}>开始训练</Button></Form></ProCard></Col><Col xs={24} lg={14}><div className="page-stack">{dataset ? <TrainingResourceHint dataset={dataset} /> : <Alert type="warning" showIcon message="请先创建数据集" />}<ProCard title="就绪检查"><Descriptions column={1} bordered size="small"><Descriptions.Item label="数据集">{dataset?.name || '-'}</Descriptions.Item><Descriptions.Item label="触发词">{dataset?.trigger || '-'}</Descriptions.Item><Descriptions.Item label="参与训练图片">{dataset?.selectedImages ?? 0} / {dataset?.totalImages ?? 0}</Descriptions.Item><Descriptions.Item label="已确认 caption">{dataset?.captionLocked ?? 0}</Descriptions.Item><Descriptions.Item label="未标注">{dataset?.unannotated ?? 0}</Descriptions.Item><Descriptions.Item label="输出">local-data/training-runs/&lt;run_id&gt;/train_config.json</Descriptions.Item></Descriptions><Divider /><Alert type="info" showIcon message="提交后会直接创建 LoRA训练任务" description="系统会准备 musubi 数据集文件、实时记录训练日志，并可在任务页取消训练进程。" /></ProCard></div></Col></Row></div></PageContainer>;
}
...
function LorasPage({ datasets, loras: loraRows, refresh }) {
  ...
  async function startTraining(row) {
    ...
    try {
      await api(`/api/training/jobs/${row.runId}/start`, { method: 'POST' });
      message.success('训练任务已启动，可到任务页查看日志或取消');
      refresh?.();
    } catch (error) {
      message.error(`启动训练失败：${error.message}`);
    } finally {
  ...
  return <PageContainer title="LoRA 版本" subTitle="管理训练中或已经可用的 LoRA 版本。"><ProCard extra={<Button onClick={refresh}>刷新</Button>}><ProTable search={false} options={false} toolBarRender={false} size="middle" rowKey="id" tableLayout="fixed" scroll={{ x: 1240 }} pagination={{ pageSize: 12 }} dataSource={loraRows} locale={{ emptyText: <Alert type="info" showIcon message="还没有 LoRA 版本" description="先到训练页启动一个训练任务。" /> }} columns={[{ title: 'LoRA', dataIndex: 'name', width: 220, render: (value, row) => <Space direction="vertical" size={0}><Text strong>{value}</Text><Text type="secondary">{row.trigger}</Text></Space> }, { title: '数据集', dataIndex: 'datasetId', width: 180, render: (value) => datasetName(datasets, value) }, { title: '基础模型', dataIndex: 'baseModel', width: 140 }, { title: '图片', dataIndex: 'imageCount', width: 80 }, { title: '推荐权重', dataIndex: 'strength', width: 100 }, { title: '状态', dataIndex: 'status', width: 130, render: statusTag }, { title: '权重路径', dataIndex: 'weightPath', ellipsis: true, render: (value) => value || <Text type="secondary">待训练输出</Text> }, { title: '操作', width: 160, render: (_, row) => <Space><Button size="small" onClick={() => openEdit(row)}>编辑</Button><Button size="small" type="primary" loading={startingId === row.id} disabled={!row.runId || row.status === '训练中'} onClick={() => startTraining(row)}>重新训练</Button></Space> }]} /></ProCard><Modal title="编辑 LoRA" open={Boolean(editing)} onCancel={() => setEditing(undefined)} onOk={saveEdit} confirmLoading={saving} destroyOnClose><Form form={editForm} layout="vertical"><Form.Item name="name" label="名称" rules={[{ required: true, message: '请输入名称' }]}><Input /></Form.Item><Form.Item name="status" label="状态"><Select options={['准备中', '训练中', '可用', '失败', '已取消', '归档'].map((value) => ({ value, label: value }))} /></Form.Item><Form.Item name="strength" label="推荐权重"><InputNumber min={0} max={2} step={0.05} style={{ width: '100%' }} /></Form.Item><Form.Item name="weightPath" label="权重路径"><Input placeholder="GPU VM 训练完成后的 safetensors 路径" /></Form.Item><Form.Item name="notes" label="备注"><TextArea rows={3} /></Form.Item></Form></Modal></PageContainer>;
}
...
function ModelsPage() {
  ...
  const vllmRuntime = status?.vllm?.runtime || {};
  const summary = status?.summary || {};
  const assetPanel = <ProCard title="模型与工具" extra={<Button loading={loading} onClick={loadStatus}>刷新</Button>}><List loading={loading} dataSource={assets} renderItem={(item) => <List.Item actions={[<Button key="check" loading={checking === item.id} onClick={() => checkAsset(item.id)}>检查</Button>]}><List.Item.Meta title={<Space wrap><Text strong>{item.name}</Text>{statusTag(item.status)}{item.required ? <Tag>必需</Tag> : null}</Space>} description={<Space direction="vertical" size={2}><Text>{item.path}</Text><Text type="secondary">{item.fileCount || 0} 个文件 / {item.size || '0 B'} / {item.kind}</Text></Space>} /></List.Item>} /></ProCard>;
