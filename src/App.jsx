...
function LorasPage({ datasets, loras: loraRows, refresh }) {
  ...
  return <PageContainer title="LoRA 版本" subTitle="管理训练准备完成、训练中或已经可用的 LoRA 版本。"><ProCard extra={<Button onClick={refresh}>刷新</Button>}><ProTable search={false} options={false} toolBarRender={false} size="middle" rowKey="id" tableLayout="fixed" scroll={{ x: 1240 }} pagination={{ pageSize: 12 }} dataSource={loraRows} locale={{ emptyText: <Alert type="info" showIcon message="还没有 LoRA 版本" description="先到训练页准备一个训练任务。" /> }} columns={[{ title: 'LoRA', dataIndex: 'name', width: 220, render: (value, row) => <Space direction="vertical" size={0}><Text strong>{value}</Text><Text type="secondary">{row.trigger}</Text></Space> }, { title: '数据集', dataIndex: 'datasetId', width: 180, render: (value) => datasetName(datasets, value) }, { title: '基础模型', dataIndex: 'baseModel', width: 140 }, { title: '图片', dataIndex: 'imageCount', width: 80 }, { title: '推荐权重', dataIndex: 'strength', width: 100 }, { title: '状态', dataIndex: 'status', width: 130, render: statusTag }, { title: '权重路径', dataIndex: 'weightPath', ellipsis: true, render: (value) => value || <Text type="secondary">待训练输出</Text> }, { title: '操作', width: 160, render: (_, row) => <Space><Button size="small" onClick={() => openEdit(row)}>编辑</Button><Button size="small" type="primary" loading={startingId === row.id} disabled={!row.runId || row.status === '训练中'} onClick={() => startTraining(row)}>开始训练</Button></Space> }]} /></ProCard><Modal title="编辑 LoRA" open={Boolean(editing)} onCancel={() => setEditing(undefined)} onOk={saveEdit} confirmLoading={saving} destroyOnClose><Form form={editForm} layout="vertical"><Form.Item name="name" label="名称" rules={[{ required: true, message: '请输入名称' }]}><Input /></Form.Item><Form.Item name="status" label="状态"><Select options={['等待GPU训练', '训练中', '可用', '失败', '已取消', '归档'].map((value) => ({ value, label: value }))} /></Form.Item><Form.Item name="strength" label="推荐权重"><InputNumber min={0} max={2} step={0.05} style={{ width: '100%' }} /></Form.Item><Form.Item name="weightPath" label="权重路径"><Input placeholder="GPU VM 训练完成后的 safetensors 路径" /></Form.Item><Form.Item name="notes" label="备注"><TextArea rows={3} /></Form.Item></Form></Modal></PageContainer>;
}
