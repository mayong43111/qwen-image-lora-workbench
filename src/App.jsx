...
function AnnotationRuntimeCard() {
  ...
  return <ProCard title="标注模型与 GPU 协调" extra={<Space>{provider === 'local' ? <Tag color="green">当前启用：本地 Qwen2.5-VL</Tag> : <Tag color="blue">当前启用：云端</Tag>}<Button loading={loading} onClick={loadRuntime}>刷新</Button></Space>}><Form form={form} layout="vertical" onFinish={saveSettings} initialValues={{ provider: 'cloud' }}><Row gutter={16}><Col xs={24} md={8}><Form.Item name="provider" label="当前启用" rules={[{ required: true }]}><Select options={[{ value: 'local', label: '本地 Qwen2.5-VL-7B / vLLM' }, { value: 'cloud', label: '云端 / Azure Foundry' }]} /></Form.Item></Col><Col xs={24} md={8}><Descriptions column={1} bordered size="small"><Descriptions.Item label="本地 vLLM">{statusTag(runtime?.status || '未知')}</Descriptions.Item><Descriptions.Item label="本地模型">{local.model || '-'}</Descriptions.Item></Descriptions></Col><Col xs={24} md={8}><Descriptions column={1} bordered size="small"><Descriptions.Item label="云端 Endpoint">{cloud.endpoint || '-'}</Descriptions.Item><Descriptions.Item label="API Key">{cloud.apiKeyConfigured ? <Tag color="green">已配置</Tag> : <Tag color="orange">未配置</Tag>}</Descriptions.Item></Descriptions></Col></Row><Divider orientation="left">本地 Qwen2.5-VL</Divider><Row gutter={16}><Col xs={24} md={12}><Form.Item name={['local', 'endpoint']} label="Endpoint"><Input placeholder="http://127.0.0.1:8000/v1" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['local', 'model']} label="模型"><Input placeholder="/data/models/qwen2.5-vl-7b-instruct" /></Form.Item></Col><Col span={24}><Space wrap><Button loading={vllmAction === 'start'} onClick={() => controlVllm('start')}>启动 vLLM</Button><Button loading={vllmAction === 'stop'} onClick={() => controlVllm('stop')}>停止 vLLM</Button><Button loading={vllmAction === 'restart'} onClick={() => controlVllm('restart')}>重启 vLLM</Button></Space></Col></Row><Divider orientation="left">云端 / Azure Foundry</Divider><Row gutter={16}><Col xs={24} md={12}><Form.Item name={['cloud', 'endpoint']} label="Endpoint"><Input placeholder="https://...cognitiveservices.azure.com/" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'apiKey']} label="API Key"><Input.Password placeholder={cloud.apiKeyConfigured ? '已配置，留空则不修改' : '请输入 API key'} autoComplete="new-password" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'deployment']} label="Deployment / Model"><Input placeholder="gpt-4o" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name={['cloud', 'apiVersion']} label="API Version"><Input placeholder="2024-10-21" /></Form.Item></Col></Row><Button type="primary" htmlType="submit" loading={saving}>保存标注配置</Button></Form></ProCard>;
}
