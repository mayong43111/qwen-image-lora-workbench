...
必须包含字段：
{
  ...
  "中文caption建议": "必须以 trigger token 开头，用两到四句详细但精确地描述这张图作为姿势参考页/多格姿势表的版式、阅读顺序、每个姿势小图的位置、具体姿态动作、身体体态、手臂和手部动作、腿部和脚部位置、线稿或素描风格",
  "剔除原因": []
}
...
- 本数据集按“训练姿势参考页 LoRA”处理：不要把多格图当作需要拆开的单人照片，也不要把多幅小图描述成多人合照；caption 要明确写“人体姿势参考页”“多格姿势参考页”“姿势表”或类似表述，让模型学习参考页版式。
- 如果图片用于人体姿势参考页 LoRA，caption 必须同时描述页面/版式和可复现的姿势细节：纸张或白底、编号/页码/小图排列、线稿或素描风格、站立/坐姿/跪姿/蹲姿/躺姿/前倾/后仰/扭转，躯干朝向和弯曲，头部朝向，肩膀和髋部角度，手臂抬起/弯曲/支撑/交叉/下垂，手掌或手指动作，双腿伸直/弯曲/交叉/跪地/踩地，重心落点和视角。
...
- 姿势参考页 caption 至少 90 个汉字；如果有多幅姿势小图，每个可见姿势至少写一个独立分句，说明小图位置、躯干、手臂/手部、腿部/脚部和重心。
- 可以用“多幅不同姿势”作为总述，但后面必须继续逐个说明每个小图的具体体态和方位。
- 对人体素描、解剖参考或未着衣人体，只做中性、学术的数据集描述，重点写姿势、体态、线稿/素描风格，不写挑逗、性感或主观评价。
- caption 应描述可复现的视觉特征，而不是只写“一名人物在室内”这类泛化短句。
