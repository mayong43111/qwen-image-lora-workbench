...
from ..services.training_service import create_evaluation_job, create_training_job, evaluation_by_id, evaluation_result_file_response, import_evaluation_result_file, list_evaluations, list_loras, start_training_run, update_evaluation, update_evaluation_result, update_lora
...
@router.post("/training/jobs/{run_id}/start")
def api_start_training_run(run_id: str):
    try:
        return ok({"task": start_training_run(run_id)})
    except RuntimeError as error:
        return fail(str(error), 400)
